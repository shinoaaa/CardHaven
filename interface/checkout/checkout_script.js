...
function resumeOrderPayment(orderId) {
    // Pakai endpoint detail order yang sudah ada (ter-scope ke pemilik order)
...
function submitPayment() {
    ...
    fetchWithTimeout(CHECKOUT_CONTROLLER, { method: 'POST', body: fd }, 20000)
        ...
        .then(json => {
            if (json.success) {
                isSubmittingProof = false;
                // Hapus param ?resume dari URL: back/refresh setelah ini tidak boleh
                // membuka ulang step pembayaran untuk order yang sudah dibayar.
...
function goToStep3() {
    ...
    // Tombol aksi (View My Orders / Browse Catalog / Back to Home) sudah
    // disediakan statis di checkout.php — tidak perlu inject duplikat di sini.
