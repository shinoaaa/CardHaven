...
window.addEventListener('pageshow', (e) => {
    if (e.persisted && orderPlaced) window.location.reload();
});
...
function renderDirectCheckoutItems() {
    ...
    directCheckoutData.items.forEach(item => {
        ...
        list.appendChild(renderCheckoutItem(item, false, null));
    });
    ...
    renderSummaryItems(directCheckoutData.items);
    updateSummary();
    renderEventSpecialInfo(); // PANGGIL INFO SPESIAL EVENT
    checkCanOrder();
}

// FUNGSI BARU UNTUK MERENDER INFO EVENT (DISKON & ESTIMASI)
function renderEventSpecialInfo() {
    ...
    // Jika ada diskon (Promo / Preorder)
    ...
    // Jika Preorder, tampilkan estimasi tanggal sampai
...
function renderCheckoutItem(item, outOfStock = false, stok = null) {
    ...
    div.innerHTML = `
        <div class="checkout-item-img">
            <img src="${fotoSrc}" alt="${escapeHtml(item.nama_produk)}" style="${outOfStock ? 'filter: grayscale(1);' : ''}">
        </div>
        <div class="checkout-item-info">
            <div class="checkout-item-name">${escapeHtml(item.nama_produk)}</div>
            <div class="checkout-item-meta">${fmt(item.harga_produk)} × ${item.jumlah_barang}</div>
            ${stockWarn}
        </div>
