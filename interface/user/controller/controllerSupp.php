        ...
        switch ($action) {
            ...
            case 'addSupplier':
                $stmt = sqlsrv_query($conn, "{CALL dbo.sp_ManageSupplier('add', 0, ?, ?, ?, ?, 1)}", 
                    [trim($_POST['nama_suplier']), trim($_POST['email']), trim($_POST['no_telp']), trim($_POST['alamat'])]);
            ...
            case 'updateSupplier':
                ...
                $stmt = sqlsrv_query($conn, "{CALL dbo.sp_ManageSupplier('edit', ?, ?, ?, ?, ?, 1)}", 
                    [$id, trim($_POST['nama_suplier']), trim($_POST['email']), trim($_POST['no_telp']), trim($_POST['alamat'])]);
                if ($stmt === false) throw new Exception(sqlsrv_errors()[0]['message']);
            ...
            case 'deleteSupplier':
                ...
                $stmt = sqlsrv_query($conn, "{CALL dbo.sp_ManageSupplier('delete', ?, '', '', '', '', 0)}", [$id]);
                if ($stmt === false) throw new Exception(sqlsrv_errors()[0]['message']);
                jsonOut(true, 'Supplier deleted successfully.');

            case 'updateSupplierStatus':
                $body = json_decode(file_get_contents('php://input'), true) ?? $_POST;
                $id = (int)($body['id_supplier'] ?? 0);
                $aktif = (int)($body['aktif'] ?? 0);
                $stmt = sqlsrv_query($conn, "{CALL dbo.sp_ManageSupplier('toggle_status', ?, '', '', '', '', ?)}", [$id, $aktif]);
                if ($stmt === false) throw new Exception(sqlsrv_errors()[0]['message']);
