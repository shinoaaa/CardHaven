<?php
/**
 * interface/user/controller/controllerCustomer.php
 * Manajemen pengguna role 0 (Customer).
    ...
    try {
        switch ($action) {
            ...
            case 'addCustomer':
                ...
                jsonOut(true, 'Customer created successfully.');
            ...
            case 'updateCustomer':
                ...
                if (!empty($_POST['password'])) {
                    $pw = password_hash($_POST['password'], PASSWORD_DEFAULT);
                    sqlsrv_query($conn, "{CALL dbo.sp_ManagePengguna('change_password', ?, ?, '', '', '', ?, ?)}", [$id, $role, $pw, $actorId]);
                }
