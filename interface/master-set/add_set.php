...
try {
    ...
    $createdBy    = isset($_POST['created_by'])    ? (int)$_POST['created_by']     : 1; // nanti diganti session
    ...
    $sqlCek    = "SELECT COUNT(*) AS cnt FROM dbo.set_kartu WHERE kode_set = ?";
    $stmtCek   = sqlsrv_query($conn, $sqlCek, [$kodeSet]);
    $rowCek    = sqlsrv_fetch_array($stmtCek, SQLSRV_FETCH_ASSOC);
    if ((int)$rowCek['cnt'] > 0) {
        sqlsrv_free_stmt($stmtCek);
        echo json_encode(["status" => "error", "target" => "kode_set", "message" => "Kode set sudah digunakan."]);
        exit;
    }
    sqlsrv_free_stmt($stmtCek);
    ...
    $sql = "INSERT INTO dbo.set_kartu
                (id_game, nama_set, kode_set, tanggal_rilis, created_by, created_date, aktif)
            VALUES
                (?, ?, ?, ?, ?, GETDATE(), 1)";

    $params = [$idGame, $namaSet, $kodeSet, $tanggalParam, $createdBy];
    $stmt   = sqlsrv_query($conn, $sql, $params);

    if (!$stmt) {
        $err = sqlsrv_errors();
        throw new Exception("Insert error: " . $err[0]['message']);
    }

    sqlsrv_free_stmt($stmt);
    sqlsrv_close($conn);

    echo json_encode(["status" => "success", "message" => "Set berhasil ditambahkan."]);
